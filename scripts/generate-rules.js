...
const originalAccountParams = {
  ...
  "pl-14": {
    parameter: "CHILDREN_SUM",
    dependencies: ["人件費", "物流費", "減価償却費", "のれん償却費", "その他販管費"]
  },
  "pl-15": {
    parameter: "CALCULATION",
...
function generateRules(accountsMaster) {
  ...
  for (const [accountId, paramInfo] of Object.entries(originalAccountParams)) {
    ...
    switch (paramInfo.parameter) {
          ...
          for (const depName of paramInfo.dependencies) {
            ...
            if (depId) {
              ...
              if (depAccount) {
                ...
                const beforePattern = new RegExp(`[\\s\\-]${depName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[\\s\\-]`);
                if (beforePattern.test(formula) || formula.includes(`- ${depName}`)) {
                  sign = -1;
                }
      ...
      case "PERCENTAGE":
        if (paramInfo.parameterValue !== undefined && paramInfo.dependencies?.[0]) {
